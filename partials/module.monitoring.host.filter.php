...
/**
 * Hosts | Components filter partial.
 *
 * Cloned from the Zabbix 7.4 core partial `monitoring.host.filter` with the problem-oriented controls removed
 * (Severity, Show suppressed problems). This module does not compute or display problem counts, so those filters
 * no longer apply. Keep the rest of the host filter (name, groups, IP/DNS/port, status, tags, maintenance) intact.
 *
 * @var CPartial $this
 */
