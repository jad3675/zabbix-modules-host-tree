...
function addGroupRow($data, &$rows, $group_name, $parent_group_name, $level, &$child_stat) {
	...
	foreach ($group['hosts'] as $hostid) {
		...
		// Component tree-view expander: lazy-loads this host's grouped items on click.
		$component_toggle = (new CSimpleButton())
			->addClass(ZBX_STYLE_TREEVIEW)
			->addClass('js-component-toggle')
			->setAttribute('data-hostid', $host['hostid'])
			->addItem((new CSpan())->addClass(ZBX_STYLE_ARROW_RIGHT));

		$col1 = new CCol();
		for($i = 0; $i <= (5 + $level*5); $i++) {
			$col1 -> addItem(NBSP_BG());
		}
		$col1 -> addItem($component_toggle);
