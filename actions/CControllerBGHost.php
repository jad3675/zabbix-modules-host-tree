...
abstract class CControllerBGHost extends CController {
	...
	protected function getData(array $filter): array {
		...
		// Count dashboards for all visible hosts in a single API call. hostdashboard.get returns one row per
		// (host, dashboard) pair and includes the 'hostid', so we can tally per host without an N+1 loop.
		$host_dashboard_count = array_fill_keys(array_keys($hosts), 0);
		$host_dashboards = API::HostDashboard()->get([
			'output' => ['dashboardid', 'hostid'],
			'hostids' => array_keys($hosts)
		]);
		foreach ($host_dashboards as $host_dashboard) {
			if (array_key_exists($host_dashboard['hostid'], $host_dashboard_count)) {
				$host_dashboard_count[$host_dashboard['hostid']]++;
			}
		}

		foreach ($hosts as &$host) {
			$host['dashboards'] = $host_dashboard_count[$host['hostid']];
